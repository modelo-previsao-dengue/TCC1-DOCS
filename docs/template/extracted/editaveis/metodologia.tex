...
\begin{itemize}
    ...
    \item Temperatura do ar: mínima, média e máxima (°C)
...
\begin{itemize}
    ...
    \item Temperaturas: média aritmética semanal (°C/semana)
...
\textbf{Modelos de Deep Learning}:
\begin{itemize}
    \item \textbf{LSTM (Long Short-Term Memory)}: redes neurais recorrentes com células de memória de longo prazo, configuradas com múltiplas camadas, dropout temporal, normalização de séries e camada densa final
    \item \textbf{BiLSTM (Bidirectional LSTM)}: extensão do LSTM processando a sequência em ambas as direções (passado e futuro)
    \item \textbf{CNN-LSTM}: arquitetura híbrida combinando blocos convolucionais unidimensionais para extração de padrões locais com LSTM para capturar dependências de longo prazo
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{MAE (Mean Absolute Error)}: mede a média dos valores absolutos dos erros, fornecendo interpretação direta em unidades de casos
\end{itemize}

\subsubsection{Métricas de Erro Relativo}

\begin{itemize}
    \item \textbf{MAPE (Mean Absolute Percentage Error)}: mede o erro percentual médio, com implementação de salvaguardas para evitar divisão por zero em semanas com casos muito baixos (substituição por valor mínimo de 1 caso)
\end{itemize}
...
\subsubsection{Teste de Estacionariedade (ADF)}

\begin{itemize}
    \item \textbf{Objetivo}: Verificar se série temporal é estacionária (pré-requisito para modelos SARIMA/SARIMAX)
    \item \textbf{Método}: Teste de Dickey-Fuller Aumentado (Augmented Dickey-Fuller)
    \item \textbf{Hipótese Nula}: Série é não-estacionária (possui raiz unitária)
    \item \textbf{Ação}: Se série não-estacionária, aplicar diferenciação (differencing) antes de ajustar modelos
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Validação Física}: Verificação se outliers estão dentro de limites fisicamente plausíveis (temperatura: -10°C a 50°C para Brasília; precipitação: não negativa, valores máximos razoáveis; umidade: 0\% a 100\%; pressão: 850 hPa a 950 hPa para altitude de ~1100m)
...
\begin{itemize}
    \item \textbf{Métodos}: Teste de Shapiro-Wilk (para amostras pequenas) ou Kolmogorov-Smirnov (para amostras maiores)
    \item \textbf{Objetivo}: Verificar se dados seguem distribuição normal (relevante para correlação de Pearson)
    \item \textbf{Interpretação}: Se normalidade rejeitada, preferir correlação de Spearman (não-paramétrica)
\end{itemize}
